...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: updated, error: updateError } = await serviceClient
    .from('community_submissions')
    .update({ status: 'declined', reviewed_at: new Date().toISOString(), feedback: notes ?? null })
    .eq('id', submissionId)
    .select()
    .maybeSingle()
  ...
  if (updateError) {
    return NextResponse.json(
      { error: `Unable to decline submission: ${updateError.message}` },
      { status: 500 },
    )
  }

  await recordEvent(submissionId, 'community_submission', 'declined', { notes }, profile.id)

  try {
    await serviceClient.functions.invoke('community-author-program-notify', {
      body: {
        type: 'community-submission-declined',
        submissionId,
    ...
    console.error('Submission decline notification failed', error)
  }
